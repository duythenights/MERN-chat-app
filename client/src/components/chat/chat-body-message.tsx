...
import { formatChatTime, resolveEntityId } from "@/lib/helper";
...
const ChatMessageBody = memo(({ message, onReply }: Props) => {
  ...
  const currentId = resolveEntityId(user);
  const senderId = resolveEntityId(message.sender);
  const isCurrentUser = Boolean(
    currentId && senderId && currentId === senderId
  );
  const senderName = isCurrentUser ? "You" : message.sender?.name;

  const replySenderId = resolveEntityId(message.replyTo?.sender);
  const replySendername =
    currentId && replySenderId && currentId === replySenderId
      ? "You"
      : message.replyTo?.sender?.name;

  const containerClass = cn(
    "group flex w-full gap-2 py-3 px-4",
